...
public:
    ...
    Quaternion(double a,double b,double c,double d): Number(a){ setI(b); setJ(c); setK(d); }
    double i() const noexcept { return i_; }
    double j() const noexcept { return j_; }
    double k() const noexcept { return k_; }
    void setI(double v){ if(!std::isfinite(v)) throw std::invalid_argument("bad component"); i_=v; }
    void setJ(double v){ if(!std::isfinite(v)) throw std::invalid_argument("bad component"); j_=v; }
    void setK(double v){ if(!std::isfinite(v)) throw std::invalid_argument("bad component"); k_=v; }
